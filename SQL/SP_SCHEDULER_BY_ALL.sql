ALTER PROCEDURE [dbo].[SCHEDULER_BY_ALL] 
	@filter int,
	@iduser int
AS
BEGIN
	...
	BEGIN
		 SELECT 
			  ...
			  INNER JOIN [dbo].[group] g ON g.idGroup =  gs.idGroup And g.groActive = 1
			  INNER JOIN season sa ON sa.idSeason = g.SeasonId and sa.seaActive = 1
			  INNER JOIN locations l ON l.idLocation = s.idLocation and l.LocActive = 1
	END
	ELSE
	BEGIN
		 SELECT 
